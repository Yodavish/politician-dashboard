...
class TestTransactionDateAnomalies:
    def test_cohen_sony_source_anomaly_is_detected(self) -> None:
        ...
        # 01/21/2026 and after signature 02/09/2026.
        violations = transaction_date_anomalies(
            _filing(), _transaction(date(2026, 12, 26), date(2026, 1, 21))
        )
        assert AFTER_NOTIFICATION in violations
        assert AFTER_FILING in violations

    def test_normal_transaction_is_not_flagged(self) -> None:
        violations = transaction_date_anomalies(
            _filing(), _transaction(date(2026, 1, 5), date(2026, 1, 21))
        )
        assert violations == []
    ...
    def test_alert_does_not_require_filing_date(self) -> None:
        ...
        violations = transaction_date_anomalies(
            filing, _transaction(date(2026, 12, 26), date(2026, 1, 21))
        )
