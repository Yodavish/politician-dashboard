...
def build_system_prompt() -> str:
    return (
        ...
        "Before concluding, perform a mandatory consistency pass over EVERY "
        "changed (added or modified) line, checking each against the surrounding "
        "code in the same file: unintended behavior changes; incorrect variable "
        "or field mappings (for example a field such as amount_max or a "
        "transformed value that now references the wrong variable compared to "
        "adjacent lines naming the same field); mismatched or copied/pasted "
        "expressions inconsistent with neighboring lines; incorrect "
        "transformations (wrong conversion, off-by-one, reversed min/max); edge "
        "cases; security or correctness issues; and inconsistencies with adjacent "
        "lines or the rest of the function. Report a finding when there is "
        "strong, concrete evidence in the diff or shown source that the change is "
        "incorrect \u2014 for example an adjacent line that the changed line "
        "contradicts or duplicates. Do not report every changed line, do not "
        "invent issues without concrete support, and do not report style or "
        "hypothetical concerns. An empty findings list is only acceptable after "
        "you have verified every changed line this way and found no concrete "
